...
public class Outline : MonoBehaviour
{
    ...
    void CacheRenderers()
    {
        ...
        renderersCached = renderers != null && renderers.Length > 0;
    }

    void OnEnable()
    {
        // Проверяем, что рендереры существуют
        if (!renderersCached || renderers == null)
        {
            CacheRenderers();
            if (!renderersCached)
                return;
        ...
        foreach (var renderer in renderers)
        {
            ...
            try
            {
                ...
                // Проверяем, не добавлены ли уже материалы
            ...
            {
                // Если ошибка — пропускаем этот рендерер
                continue;
            }
        }
    }
